...
patch_model_path="./data_cache/flower_a_patch.pt"
visualize_patchnet=False
if_train_r=True
continue_train_r=False
raw_model_path="./data_cache/flower_a_raw.pt"
######################
...
model_p = Shallow_ext(2, 50, 1).to(device)
print("patch model:\n",model_p)
...
patchnet = lambda x: functional_call(model_p,func_params_p,x.to(device))
...
def phi1(x):
    ...
    return torch.tanh(x[0]**2+x[1]**2 -r(theta)**2)
    # return x[0]**2+x[1]**2 -r(theta)**2

def af1(x):
    phi=phi1(x)
    return torch.abs(phi)*phi
...
model_r = Shallow_Deep_ext(2, 25, 1,2 ,addiction_features=[af1]).to(device)
func_params_r= dict(model_r.named_parameters())
model_r.to("meta")


if if_train_r:
    data_r = databuilderforBase(g_base,patchnet,reference_u,n_inner=1600,n_boundary=120,random_method="Hammersley",device=device)
...
rawnet = lambda x: vmap(functional_call,(None,None,0))(model_r,func_params_r,x.to(device)) # call vmap because model_r.phi's input is a single instance
...
def f(x:np.ndarray):
    ...
    ans[patch_index] += patch_laplace(torch.tensor(x[patch_index]),patchnet,device=device).detach().cpu().numpy()
    return ans

resolution = 1024
U_pred = FFT_poisson2D_fast_solver(f,boundary,resolution)
eval_2d_FD(U_pred,patchnet,g_base,reference_u,resolution,device)
