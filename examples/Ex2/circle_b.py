...
patch_model_path="./data_cache/circle_b_patch.pt"
visualize_patchnet=False
if_train_r=True
continue_train_r=False
raw_model_path="./data_cache/circle_b_raw.pt"
######################
...
if visualize_patchnet:
    ...
    v=patchnet(torch.tensor(x).to(device)).detach().cpu().numpy()
...
def f(x:np.ndarray):
    ...
    ans[patch_index] += patch_laplace(torch.tensor(x[patch_index]).to(device),patchnet).detach().numpy()
    return ans
