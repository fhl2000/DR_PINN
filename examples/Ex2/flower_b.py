...
patch_model_path="./data_cache/flower_b_patch.pt"
visualize_patchnet=False
if_train_r=True
continue_train_r=False
raw_model_path="./data_cache/flower_b_raw.pt"
######################
...
if if_train_r:
    data_r = databuilderforBase(g_base,patchnet,reference_u,n_boundary=120,n_inner=1600,random_method="Hammersley",device=device)
...
# using a FD fast solver for stage 2
...
def f(x:np.ndarray):
    ...
    ans[patch_index] += patch_laplace(torch.tensor(x[patch_index]),patchnet).detach().cpu().numpy()
    return ans

resolution = 1024  # max 1024
