...
# Local imports
from . import mlcompat as ml
from . import utils as dmutils

__all__ = ['distmesh2d','distmeshnd']
...
def distmeshnd(fd, fh, h0, bbox, it_max=1000, pfix=None, fig='gcf'):
    """
    ...
    if fig is not None:
        ...
        if dim == 2:
            ...
            from .plotting import axes_simpplot3d
            fig.clf()
    ...
    count = 0                                        # count for retriangulation
    pold = float('inf')                              # For first iteration

    for iters in range(it_max):
        ...
        if maxdp < ptol*h0:
            break
        if iters %50 ==0:
            print(f'it{iters}: n_retri={count}, maxdp={maxdp:.5e}')
