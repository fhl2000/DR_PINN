...
def bndproj(p, t, fd):
    ...
    if dim == 2:
        ...
        d = fd(p[tri])
        dgradx = (fd(p[tri]+[deps,0,0])-d)/deps
        dgrady = (fd(p[tri]+[0,deps,0])-d)/deps
        dgradz = (fd(p[tri]+[0,0,deps])-d)/deps
        dgrad2 = dgradx**2 + dgrady**2 + dgradz**2
        dgrad2[dgrad2 == 0] = 1
        p[tri] -= (d*np.vstack((dgradx, dgrady, dgradz))/dgrad2).T
...
def fixmesh(p, t, ptol=2e-13):
    """Remove duplicated/unused nodes and fix element orientation. Only for 2d
